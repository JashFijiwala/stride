...
export function SettingsClient({ userId, email }: { userId: string; email: string }) {
  ...
  async function exportJSON(data: Awaited<ReturnType<typeof fetchAllData>>) {
    ...
    triggerDownload(
      ...
      `haven-data-${format(new Date(), 'yyyy-MM-dd')}.json`,
      'application/json'
    )
  }
  ...
  async function exportCSV(data: Awaited<ReturnType<typeof fetchAllData>>) {
    ...
    triggerDownload(
      ...
      `haven-journal-${format(new Date(), 'yyyy-MM-dd')}.csv`,
      'text/csv'
    )
  }
  ...
  async function exportTXT(data: Awaited<ReturnType<typeof fetchAllData>>) {
    ...
    triggerDownload(
      lines.join('\n'),
      `haven-journal-${format(new Date(), 'yyyy-MM-dd')}.txt`,
      'text/plain'
    )
  }
  ...
  async function exportPDF(data: Awaited<ReturnType<typeof fetchAllData>>) {
    ...
    doc.text('Haven Journal Export', margin, 18)
    y = 36
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      doc.text(
        `Generated by Haven  ·  Page ${i} of ${totalPages}`,
        pageW / 2,
    ...
    doc.save(`haven-report-${format(new Date(), 'yyyy-MM-dd')}.pdf`)
  }
    ...
    <div className="mx-auto max-w-2xl space-y-4 px-4 py-6">
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5">
        ...
        <p className="mb-4 text-xs text-[var(--text-muted)]">
          Haven uses these to give you more relevant wellbeing insights and classifications.
        </p>
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)]">
        ...
        >
          <span className="text-sm font-medium text-[var(--text-primary)]">About Haven</span>
          {showAbout ? (
          ...
          {showAbout && (
            ...
            >
              <div className="border-t border-[var(--border)] px-5 pb-5 pt-4 space-y-3 text-sm leading-relaxed text-[var(--text-secondary)]">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-[var(--text-primary)]">Haven</span>
      ...
      {/* Version */}
      <p className="pb-4 text-center text-xs text-[var(--text-muted)]">Haven v1.0.0</p>
